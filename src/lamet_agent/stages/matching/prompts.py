...
2. load_quasi_pdf on metadata.matching.quasi_input (default the Fourier artifacts/quasi_pdf.npz). It auto-detects the Fourier EnsembleData npz and loads the full per-sample quasi-PDF (every bootstrap/jackknife sample), taking the real part; pass component='im' only if the quasi-PDF lives in the imaginary channel. Stores x_ls and quasi_ed.
...
4. apply_matching -> lightcone_ed: applies kernel_matrix to each quasi-PDF sample (lightcone_i = kernel_matrix @ quasi_i) and stores the matched samples as an EnsembleData.
