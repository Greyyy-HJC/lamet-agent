...
本仓库将物理 scheme 与实现 strategy 分开：`scheme` 为 `ratio`、`hybrid` 或 `msbar`，renormalization `strategy` 为 `external_denominator` 或 `self_renormalization`。`msbar + external_denominator` 与 ratio 一样做点除，`denominator` 可以是上游 job id，也可以是一个非零常数（所有裸矩阵元都除以该常数）。`hybrid + self_renormalization` 在短程使用 target/denominator，在长程使用 target/$(Z_R Z_T)$，其中 $Z_T$ 在最接近 $z_s$ 的网格点按每个重采样样本确定，以保证连续性。
