...
def expand(
    ...
    if set(config) != {"propagate_fourier_variants", "scale_variants"}:
        raise ValueError(
            "perturbative_matching systematics keys must be propagate_fourier_variants and scale_variants"
        )
    ...
    for index, variant in enumerate(raw_variants):
        if not isinstance(variant, Mapping) or set(variant) != {"id", "factor"}:
            raise ValueError(
                f"perturbative_matching.scale_variants[{index}] must contain id and factor"
            )
        label = variant["id"]
    ...
    if propagate and (not fourier_mapping or not fourier_labels):
        raise ValueError(
            "Matching requested Fourier propagation but no Fourier variants were compiled"
        )
    labels = ([*fourier_labels] if propagate else []) + [
        label for label, _ in scale_variants
    ]
    ...
    if suffixes and any(
        str(job.get("id", "")).endswith(suffixes) for job in central
    ):
        raise ValueError(
            "Matching systematics cannot be combined with explicitly authored variation jobs"
        )
    known_ids = {
    ...
    for job in central:
        ...
        if not isinstance(central_fourier_id, str):
            raise ValueError(
                f"Matching job '{job['id']}' systematics require one upstream Fourier job input"
            )
        mapping[job["id"]] = {}
        if propagate:
            if central_fourier_id not in fourier_mapping:
                raise ValueError(
                    f"Matching job '{job['id']}' references Fourier job without compiled variants"
                )
            for label in fourier_labels:
                clone = copy.deepcopy(job)
                clone["id"] = f"{job['id']}_{label}"
                clone["inputs"]["quasi"] = fourier_mapping[central_fourier_id][
                    label
                ]
                if clone["id"] in known_ids:
                    raise ValueError(
                        f"generated Matching job id collides with '{clone['id']}'"
                    )
        ...
        if not isinstance(mu, (int, float)) or isinstance(mu, bool):
            raise ValueError(
                f"Matching job '{job['id']}' requires a numeric central mu"
            )
