...
def run(
    ...
    *,
    ...
    for record in records:
        ...
        if all(
            values is None or any(value in tags.get(key, []) for value in values) for key, values in filters.items()
        ):
            selected.append(
                {key: record.get(key) for key in ("id", "title", "authors", "year", "abstract", "source", "tags")}
            )
    context.state["literature_candidates"] = selected
    return {
        "summary": f"listed {len(selected)} literature records",
        "metrics": {"paper_ids": [record["id"] for record in selected]},
        "state_keys": ["literature_candidates"],
        "artifacts": [],
    }
