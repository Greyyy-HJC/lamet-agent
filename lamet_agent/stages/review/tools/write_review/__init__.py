...
def run(context: ToolContext, *, title: str, analysis: str, conclusion: str) -> dict[str, object]:
    ...
    lines = [
        ...
        "## Scoped results",
        "",
        f"Inspected {len(context.state['result_summary'])} explicitly referenced results.",
        "",
    ...
    lines.extend(
        [
            f"- `{finding['field']}` mismatch at result {finding['index']}: {finding['left']!r} != {finding['right']!r}"
            for finding in findings
        ]
        or ["- No deterministic mismatches were found."]
    )
