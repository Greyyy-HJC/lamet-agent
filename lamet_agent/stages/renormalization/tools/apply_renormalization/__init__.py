...
def run(context: ToolContext) -> dict[str, object]:
    ...
    if context.params.get("operation") != "apply":
        raise ValueError("apply_renormalization requires operation='apply'")
    ...
    if strategy == "self_renormalization":
        ...
        if factor.attrs.get("scale_gev") is not None and abs(float(factor.attrs["scale_gev"]) - float(context.params.get("mu", factor.attrs["scale_gev"]))) > 1e-12:
            raise ValueError("self-renormalization factor scale does not match the apply job")
        if context.params.get("normalization"):
            target = normalize_at_origin(target)
            factor = normalize_at_origin(factor)
        remap = context.params["factor_remap"]
        ...
        lambda_qcd = float(remap["LambdaQCD_gev"])
        log_a_lambda = np.log(float(spacing) * lambda_qcd / HBAR_C_GEV_FM)
        remapped_factor = mean_factor * (1.0 + float(remap["d"]) / log_a_lambda) / (1.0 + float(d_from) / log_a_lambda) * np.exp((float(remap["m0_gev"]) - float(m0_from)) * z_factor / HBAR_C_GEV_FM)
        ...
        if np.any(long_distance):
            ...
            baseline = log_m(z_factor, float(spacing), k=k, lambda_qcd_gev=lambda_qcd, d=float(remap["d"]), n_f=n_f, scale_gev=scale_gev) + float(remap["m0_gev"]) * z_factor / HBAR_C_GEV_FM
            ...
            completed_log = log_m(z_long, float(spacing), k=k, lambda_qcd_gev=lambda_qcd, d=float(remap["d"]), n_f=n_f, scale_gev=scale_gev) + float(remap["m0_gev"]) * z_long / HBAR_C_GEV_FM + np.polyval(coefficients, z_long) * float(spacing)
            denominator_values[np.flatnonzero(nonzero)[long_distance]] = np.exp(completed_log)
        if scheme == "ratio":
            denominator_values[nonzero] *= np.exp(zmsbar_pdf_log(np.abs(z_target[nonzero]), scale_gev=float(context.params["mu"]), model=str(remap["zms_model"])))
        ...
        if scheme == "hybrid":
            ...
            switch = float(context.params["zs_fm"])
        ...
        if isinstance(denominator_source, (int, float)) and not isinstance(denominator_source, bool):
            ...
            if scheme == "hybrid":
                result = hybrid_ratio(target, denominator, zs_fm=float(context.params["zs_fm"]), delta_m_gev=float(context.params["delta_m_gev"]), m0_gev=float(context.params["m0_gev"]))
            else:
    ...
    diagnostics = {"scheme": scheme, "strategy": strategy, "sample_count": result.n_sample, "dims": result.dims}
    ...
    summary = {"stage_id": context.stage_id, "job_id": context.job_id, "result": "renormalized_matrix_element", "decisions": {"scheme": scheme, "strategy": strategy, "normalization": bool(context.params["normalization"])}, "diagnostics": diagnostics, "artifacts": ["output.nc", "diagnostics/renormalization.json", "plots/result.pdf", "report.md"]}
