...
from lamet_agent.stages.renormalization.physics import divide_by_constant, hybrid_ratio, log_m, normalize_at_origin, ratio, zmsbar_pdf_log
...
def run(context: ToolContext) -> dict[str, object]:
    ...
    if strategy == "self_renormalization":
        ...
        if "a" in factor.dims:
            matches = [index for index, value in enumerate(factor.coords["a"]) if abs(float(value) - float(spacing)) <= 1e-12]
            if len(matches) != 1:
        ...
        if factor.attrs.get("scale_gev") is not None and abs(float(factor.attrs["scale_gev"]) - float(params.get("mu", factor.attrs["scale_gev"]))) > 1e-12:
            raise ValueError("self-renormalization factor scale does not match the apply job")
        if params["normalization"]:
            target = normalize_at_origin(target)
            factor = normalize_at_origin(factor)
        target_observable = str(context.manifest["metadata"]["target_observable"]).lower()
        try:
            zms_model = {"pdf": "pdf_nlo", "da": "da_nlo"}[target_observable]
        except KeyError as exc:
            raise ValueError(
                "self-renormalization supports only PDF or DA ZMSbar conversion"
            ) from exc
        d_from = factor.attrs.get("d")
        m0_from = factor.attrs.get("m0_gev")
        if not isinstance(d_from, (int, float)) or isinstance(d_from, bool) or not isinstance(m0_from, (int, float)) or isinstance(m0_from, bool):
        ...
        remapped_factor = mean_factor * (1.0 + d_to / log_a_lambda) / (1.0 + float(d_from) / log_a_lambda) * np.exp((m0_to - float(m0_from)) * z_factor)
        ...
        if np.any(long_distance):
            ...
            baseline = log_m(z_factor, float(spacing), k=k, lambda_qcd_gev=lambda_qcd, d=d_to, n_f=n_f, scale_gev=scale_gev) + m0_to * z_factor
            ...
            completed_log = log_m(z_long, float(spacing), k=k, lambda_qcd_gev=lambda_qcd, d=d_to, n_f=n_f, scale_gev=scale_gev) + m0_to * z_long + np.polyval(coefficients, z_long) * float(spacing)
        ...
        if scheme == "hybrid":
            ...
            values = [np.where(mask, np.asarray(short_sample), np.asarray(long_sample)) for short_sample, long_sample in zip(short.values, long.values)]
            attrs = short.attrs
            attrs.update({"hybrid_switch_coord_fm": switch_coord, "strategy": strategy})
            result = EnsembleData(target.ensemble, target.resample, values, target.dims, target.coords, attrs=attrs, name="renormalized_matrix_element")
        else:
        ...
        if isinstance(denominator_source, (int, float)) and not isinstance(denominator_source, bool):
            ...
            if scheme == "hybrid":
                result = hybrid_ratio(target, denominator, zs_fm=float(params["zs_fm"]), delta_m_gev=float(params["delta_m_gev"]), m0_gev=float(params["m0_gev"]))
            else:
                result = ratio(target, denominator)
    attrs = result.attrs
    attrs.update({"operation": "apply", "renormalization_scheme": scheme, "strategy": strategy, "coord_unit": "fm", "input_coord_unit": attrs.get("input_coord_unit", "fm"), "units": json.dumps({"values": "dimensionless", **{dim: "fm" for dim in result.dims}})})
    result = EnsembleData(result.ensemble, result.resample, [np.asarray(sample) for sample in result.values], result.dims, result.coords, attrs=attrs, name="renormalized_matrix_element")
    ...
    diagnostics = {"scheme": scheme, "strategy": strategy, "sample_count": result.n_sample, "dims": result.dims, "z_range_fm": [float(np.min(z_values)), float(np.max(z_values))], "z_count": int(z_values.size), "normalization": normalize_inputs, "zs_fm": params.get("zs_fm"), "m0_gev": params.get("m0_gev"), "delta_m_gev": params.get("delta_m_gev"), "d": params.get("d"), "mu": params.get("mu"), "LambdaQCD_gev": params.get("LambdaQCD_gev"), "target_samples": target.n_sample, "denominator_kind": "constant" if isinstance(context.inputs.get("denominator"), (int, float)) and not isinstance(context.inputs.get("denominator"), bool) else "matrix" if "denominator" in context.inputs else "self_factor", "input_z_ranges_fm": {role: [float(np.min(value.coords["z"])), float(np.max(value.coords["z"]))] for role, value in aligned.items() if isinstance(value, EnsembleData) and "z" in value.coords}}
    (context.artifact_directory / "diagnostics").mkdir(exist_ok=True)
    (context.artifact_directory / "diagnostics" / "renormalization.json").write_text(json.dumps(diagnostics, indent=2), encoding="utf-8")
    start_plot()
    ...
    errorbar(result.coords["z"], plot_data.average(sample_error_mode))
    ...
    summary = {"stage_id": context.stage_id, "job_id": context.job_id, "result": "renormalized_matrix_element", "decisions": {"scheme": scheme, "strategy": strategy, "normalization": bool(params["normalization"])}, "diagnostics": diagnostics, "artifacts": ["output.nc", "diagnostics/renormalization.json", "plots/result.pdf", "report.md"]}
    context.finish(result, summary)
    return {"summary": "published renormalized matrix element", "metrics": diagnostics, "state_keys": [], "artifacts": summary["artifacts"]}
