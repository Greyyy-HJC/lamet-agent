...
from lamet_agent.plotting import band, bar, configure_plot, line, save_figure, series_color, start_plot
...
def run(context: ToolContext) -> dict[str, object]:
    ...
    start_plot()
    bar(x, total_error, width=0.078, color=series_color(0), label="total error")
    bar(x, total_systematic, width=0.06, color=series_color(1), label="total systematic")
    configure_plot(xlabel=r"$x$", ylabel="distribution uncertainty", legend=True)
    save_figure(context.artifact_directory / "plots" / "systematics_budget.pdf")

    start_plot()
    band(x, central - total_error, central + total_error, color=series_color(1), label="total error")
    band(x, central - stat_sdev, central + stat_sdev, color=series_color(0), label="stat")
    line(x, central, color=series_color(0), linewidth=1.6)
