...
def run(context: ToolContext) -> dict[str, object]:
    ...
    if not isinstance(momentum_dependence, dict) or set(momentum_dependence) != {f"{float(value):g}" for value in params["pdep_gev"]}:
    ...
    for record in momentum_dependence.values():
        errorband(
            ...
            label=f"Pz={float(record['momentum_gev']):g} GeV",
        )
    errorband(data.coords["x"], plot_data.average(sample_error_mode), label="Pz→∞")
    configure_plot(xlabel="x", ylabel="physical distribution", legend=True)
    save_figure(
    ...
    summary = {"stage_id": context.stage_id, "job_id": context.job_id, "result": "physical_distribution", "decisions": {"terms": data.attrs.get("extrapolation_terms"), "pdep_gev": params["pdep_gev"]}, "diagnostics": comparison, "artifacts": ["output.nc", "diagnostics/extrapolation.json", "plots/distribution.pdf", "plots/momentum_dependence.pdf", "plots/momentum_dependence.svg", "report.md"]}
