...
def run(context: ToolContext, *, t_min: int, t_max: int, n_states: int, prior_means: dict[str, float], prior_widths: dict[str, float]) -> dict[str, object]:
    ...
    source = next((value for value in correlators.values() if value.attrs.get("correlator_type") == "two_point"), next(iter(correlators.values())))
    ...
    energy_samples, fit = fit_spectrum_samples(np.asarray(source.values)[:, selection], time[selection], n_states, resample=source.resample, prior_means=prior_means, prior_widths=prior_widths, sample_error_mode=str(context.manifest["metadata"]["sample_error_mode"]), workers=context.workers, _parallel=context._parallel)
    ...
    attrs.update({"observable": "spectrum", "method": "direct_fit", "fit_energy_unit": "lattice", "sample_error_mode": context.manifest["metadata"]["sample_error_mode"], "units": '{"values":"GeV","state":"index"}'})
    candidate = EnsembleData(source.ensemble, source.resample, energy_samples, ["state"], {"state": list(range(n_states))}, attrs=attrs, name="spectrum")
    candidates = context.state.setdefault("spectrum_candidates", [])
    candidate_id = f"spectrum_{len(candidates) + 1:03d}"
    candidates.append({"id": candidate_id, "method": "direct_fit", "observable": "spectrum", "window": {"t_min": t_min, "t_max": t_max}, "data": candidate, "prior_means": dict(prior_means), "prior_widths": dict(prior_widths), **fit})
    return {"summary": f"stored spectral candidate {candidate_id}", "metrics": {"candidate_id": candidate_id, "energy_mean_gev": np.asarray(candidate.mean).tolist(), **fit}, "state_keys": ["spectrum_candidates"], "artifacts": []}
