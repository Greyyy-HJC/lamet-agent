...
PARAM_RULES = (
    ...
    Provides("", "da", "$.metadata.target_observable", physics="For target_observable=da, enables midpoint phase/symmetry projection and ordered endpoint flavor classes used to constrain meson-DA tail amplitudes; these controls do not apply to PDF or GPD jobs."),
    Provides("", "gpd", "$.metadata.target_observable", physics="For target_observable=gpd, enables paired-flow Hermitian completion for non-forward matrix elements and the phase/endpoint convention selected by phase_transfer_gpd."),
    Recommends("gpd", "phase_transfer_gpd", physics="Selects the endpoint convention used before Hermitian pairing of exchanged positive-z flows: mid_at_0 applies opposite half-transfer phases, barpsi_at_0 applies no phase transfer, and psi_at_0 conjugates the exchanged ordering; all produce one explicit signed-z input for the Fourier kernel.", default="mid_at_0"),
    ...
    Depends("scheme_scan", "sector", physics="The transform needs a partonic sector: quark-PDF/GPD sectors select the configured signed-x projections, while full retains the complete complex result; DA permits only full."),
    ...
    Recommends("scheme_scan", "q_min", physics="Preferred lsqfit-Q threshold for sample-average range selection and non-averaged per-resample model choice; after the recommendation budget is exhausted, publication falls back to the numerically valid center model with maximum Q.", default=0.05),
    ...
    Value("scheme_scan.q_min", (int, float), physics="Preferred threshold in [0,1] for the chi-square upper-tail probability Q; a numerically valid maximum-Q center model may be published below it after recommendation retries are exhausted, and evidence averaging does not apply it.", validator=_unit_interval),
    ...
    Value("gpd.phase_transfer_gpd", Literal["mid_at_0", "barpsi_at_0", "psi_at_0"], physics="'mid_at_0' multiplies primary and exchanged positive-z flows by opposite exp(+-i Delta P_z z/(2 hbar c)) phases; 'barpsi_at_0' leaves both unchanged; 'psi_at_0' conjugates and exchanges their endpoint ordering before explicit signed-z completion."),
)

INPUT_RULES = (
    Depends("", "input", physics="One coordinate-space matrix element on a nonnegative-z or signed-z grid supplies the resampled signal and provenance; PDF/DA nonnegative data use real-even/imaginary-odd completion, while a non-forward GPD can use an exchanged-flow partner for explicit signed-z completion."),
    Recommends("", "hermitian_partner", physics="Optional exchanged-flow input for a non-forward GPD; its source and sink momenta are reversed relative to input so Hermiticity supplies the negative-z branch.", default=None),
    Value("hermitian_partner", (str, dict), physics="Upstream job id or external-file object for the exchanged GPD flow; the data must share ensemble, resampling, fm z grid, and compatible samples with input."),
