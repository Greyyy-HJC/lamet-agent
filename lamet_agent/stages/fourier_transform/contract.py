...
PARAM_RULES = (
    ...
    Provides("", "gpd", "$.metadata.target_observable", physics="For target_observable=gpd, enables the bilocal anchor and paired-flow Hermiticity convention used for non-forward matrix elements."),
    Recommends("gpd", "bilocal_anchor", physics="A GPD transform needs the operator endpoint fixed at the midpoint, barred field, or unbarred field to determine its Hermiticity phase convention.", default="mid_at_0"),
    ...
    Depends("scheme_scan", "sector", physics="The transform needs a partonic sector: quark-PDF/GPD sectors select negative-x projections, while full retains the complete complex result; DA permits only full."),
    ...
    Value("scheme_scan.sector", Literal["valence", "singlet", "full"], physics="'valence' forms quark-minus-antiquark, 'singlet' forms quark-plus-antiquark, and 'full' keeps both complex channels; GPD projections are applied after a signed-y transform, while DA allows only full."),
    ...
    Value("gpd.bilocal_anchor", Literal["mid_at_0", "barpsi_at_0", "psi_at_0"], physics="'mid_at_0' centers the bilocal at the origin; 'barpsi_at_0' fixes the barred endpoint at zero; 'psi_at_0' fixes the unbarred endpoint at zero and reverses the canonical separation."),
)

INPUT_RULES = (
    Depends("", "input", physics="One coordinate-space matrix element on a nonnegative-z or signed-z grid supplies the resampled signal and its physical provenance; nonnegative PDF/DA data use the declared real-even/imaginary-odd convention."),
    Recommends("", "hermitian_partner", physics="A non-forward GPD may provide the same matrix element with initial and final momenta exchanged, allowing the negative-z branch to be completed from the paired flow.", default=None),
    Value("hermitian_partner", (str, dict), physics="An optional upstream job id or external-file object identifying the exchanged GPD flow."),
