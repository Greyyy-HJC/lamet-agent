...
def run(context: ToolContext, *, candidate_id: str) -> dict[str, object]:
    ...
    diagnostics = {"candidate_id": candidate_id, "tail_model": attrs["tail_model"], "sample_count": result.n_sample, **{key: selected[key] for key in ("chi2", "dof", "chi2_dof", "Q", "aic")}}
    (context.artifact_directory / "diagnostics").mkdir(exist_ok=True)
    (context.artifact_directory / "diagnostics" / "fourier.json").write_text(json.dumps(diagnostics, indent=2), encoding="utf-8")
    start_plot()
    ...
    summary = {"stage_id": context.stage_id, "job_id": context.job_id, "result": "quasi_distribution", "decisions": {"candidate_id": candidate_id}, "diagnostics": diagnostics, "artifacts": ["output.nc", "diagnostics/fourier.json", "plots/distribution.pdf", "report.md"]}
    context.finish(result, summary)
    return {"summary": "published quasi distribution", "metrics": diagnostics, "state_keys": [], "artifacts": summary["artifacts"]}
