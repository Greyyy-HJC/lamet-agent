...
def run(context: ToolContext, *, model_id: str, z_min_fm: float, z_max_fm: float, prior_means: dict[str, float], prior_widths: dict[str, float]) -> dict[str, object]:
    ...
    parameters, fit = fit_tail_parameters(data, model_id=model_id, z_min_fm=effective_z_min, z_max_fm=z_max_fm, prior_means=prior_means, prior_widths=prior_widths, order=order, component=str(conventions["component"]), lambda0_gev=float(scan["Lambda0_gev"]), observable=observable, psi1_flavor_class=psi1_flavor_class, psi2_flavor_class=psi2_flavor_class, sector=sector, hadron=hadron, workers=context.workers)
    candidate = extend_tail(data, z_max_fm=float(context.params["zmax_ext_fm"]), z_min_fm=effective_z_min, smoothing_method=str(context.params["smooth"]), smoothing_width_fm=z_max_fm - effective_z_min, model_id=model_id, tail_parameters=parameters, order=order, observable=observable, psi1_flavor_class=psi1_flavor_class, psi2_flavor_class=psi2_flavor_class, sector=sector, hadron=hadron)
    candidates = context.state.setdefault("tail_candidates", [])
    candidate_id = f"tail_{len(candidates) + 1:03d}"
    candidates.append({"id": candidate_id, "model_id": model_id, "z_min_fm": effective_z_min, "z_max_fm": z_max_fm, "zmax_ext_fm": context.params["zmax_ext_fm"], "smoothing_method": context.params["smooth"], "data": candidate, "parameters": parameters, "prior_means": dict(prior_means), "prior_widths": dict(prior_widths), **fit})
