...
def run(context: ToolContext) -> dict[str, object]:
    ...
    data = complete_signed_z(data, context.params["symmetry"])
    ...
    grid_values = [
        *parameters["zmin_fm"],
        *parameters["zmax_fm"],
        *parameters["smoothing"]["widths_fm"],
        parameters["zmax_ext_fm"],
    ]
    if any(not math.isclose(round(float(value) / spacing) * spacing, float(value), rel_tol=0.0, abs_tol=1e-12) for value in grid_values):
        raise ValueError("Fourier fit boundaries, smoothing widths, and tail extent must lie on the input z grid")
    ...
    return {"summary": "inspected and completed signed z grid", "metrics": context.state["tail_inspection"], "state_keys": ["fourier_input", "tail_inspection"], "artifacts": []}
