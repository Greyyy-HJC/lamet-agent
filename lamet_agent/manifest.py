...
_BASE_RULES: tuple[Depends | Provides | Recommends | Value, ...] = (
    Value("", dict, physics="The manifest groups run metadata, ordered stages, and optional systematic variants."),
    Depends("", "metadata", physics="All stages share one physical target and one run-wide sampling convention."),
    Depends("", "stages", physics="The ordered stage mapping defines the sequence of physical transformations."),
    Recommends("", "systematics", physics="Systematic variants define alternative analyses for uncertainty estimates; an empty mapping requests none.", default={}),
    Value("metadata", dict, physics="The object contains the target observable, lattice sampling, and run-wide execution settings."),
    Value("stages", dict, physics="The nonempty ordered mapping is the analysis execution graph.", validator=bool),
    Value("systematics", dict, physics="The object maps stage identifiers to their systematic-variation settings."),
    Depends("metadata", "run_id", physics="Jobs and artifacts need a stable identifier for one analysis run."),
    Depends("metadata", "root_directory", physics="Relative input paths need a common resolution base."),
    Depends("metadata", "artifacts_directory", physics="Each job needs an output directory for its artifacts and diagnostics."),
    Depends("metadata", "random_seed", physics="Resampling and stochastic fits need one reproducible random seed."),
    Depends("metadata", "workers", physics="Sample-wise numerical work needs one run-wide concurrency limit."),
    Depends("metadata", "target_observable", physics="The workflow needs the physical observable produced by its final stages."),
    Recommends("metadata", "parton", physics="The parton species selects the correlator and matching conventions; current workflows use quarks.", default="quark"),
    Depends("metadata", "resample_mode", physics="Correlator uncertainties need one consistent jackknife or bootstrap sampling plan."),
    Depends("metadata", "sample_error_mode", physics="All stages need one convention for sample centers and reported errors."),
    Recommends("metadata", "parameter_recommendation_retries", physics="A failed parameter recommendation may be retried a bounded number of times for each job.", default=1),
    Provides("metadata", "bootstrap", "metadata.resample_mode", physics="Bootstrap mode needs an explicit replica count for resampled ensembles."),
    Depends("metadata.bootstrap", "samples", physics="Bootstrap resampling needs the number of replicas to generate."),
    Depends("metadata", "bin_size", physics="Raw correlator configurations need a common bin size before resampling."),
    Value("metadata.run_id", str, physics="The human-readable identifier of this analysis run."),
    Value("metadata.root_directory", str, physics="The filesystem base directory for relative input paths."),
    Value("metadata.artifacts_directory", str, physics="The filesystem directory receiving per-job outputs and diagnostics."),
    Value("metadata.random_seed", int, physics="The nonnegative integer seed for reproducible random resampling and numerical procedures.", validator=_nonnegative),
    Value("metadata.workers", int, physics="The positive integer limit on concurrent sample-wise jobs.", validator=_positive),
    Value("metadata.target_observable", Literal["pdf", "da", "gpd"], physics="'pdf' targets a parton distribution; 'da' targets a distribution amplitude; 'gpd' targets a generalized parton distribution."),
    Value("metadata.parton", Literal["quark"], physics="'quark' selects quark correlators and quark matching conventions."),
    Value("metadata.resample_mode", Literal["jackknife", "bootstrap"], physics="'jackknife' omits each data group in turn; 'bootstrap' generates replica samples from binned configurations."),
    Value("metadata.sample_error_mode", Literal["covariance", "variance", "one_sigma"], physics="'covariance' retains sample covariance; 'variance' reports variance-only errors; 'one_sigma' uses the median and bootstrap 16/84-percentile interval."),
    Value("metadata.parameter_recommendation_retries", int, physics="The nonnegative integer number of extra LLM recommendation attempts allowed per job.", validator=_nonnegative),
    Value("metadata.bootstrap.samples", int, physics="The positive integer number of bootstrap replicas used for uncertainty estimation.", validator=_positive),
    Value("metadata.bin_size", int, physics="The positive integer number of raw configurations combined in each resampling bin.", validator=_positive),
)
# fmt: on
# ruff: enable[E501]
