...
def kernel(x_out: np.ndarray, x_in: np.ndarray, *, momentum_gev: float, scale_gev: float, zs_fm: float, kappa: float=1.0, mu_min_gev: float=0.6, eps: float=1e-12) -> np.ndarray:
    return _rgr_from_fixed_order(_fixed_order, _p_nlo_transversity, np.asarray(x_out, dtype=float), momentum_gev, scale_gev, np.asarray(x_in, dtype=float), eps, zs_fm * momentum_gev / GEV_FM, needs_zspz=True, takes_zspz=False, needs_zpsi=False, kappa=kappa, mu_min=mu_min_gev)
