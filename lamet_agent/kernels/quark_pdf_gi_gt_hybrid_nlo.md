<!-- lamet-agent formula cache; kernel=quark_pdf_gi_gt_hybrid_nlo; arxiv=2412.20461; equations=Eq. (24); digest=d2cbc5eb0db1406b; paper_used=true -->
...
- **Plus prescription**: The code's `build_matching_matrix_column_plus` restores the plus prescription by column-sum subtraction, which matches the paper's $[\,\cdot\,]_{+(1)}^{D}$ with $D$ the domain. The paper splits into three domains $[1,\infty]$, $[0,1]$, $[-\infty,0]$; the code's `C_ratio_gi` does not explicitly split but the `if eps < ksi < 1-eps` branch covers $[0,1]$ and the `else` covers the other two with the sign function. The domains are consistent.
