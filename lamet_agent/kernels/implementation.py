...
def beta(order: int=0, Nf: int=3) -> float:
    ...
    if order == 2:
        return 2857.0 / 54.0 * CA ** 3 + (2.0 * CF ** 2 - 205.0 / 9.0 * CF * CA - 1415.0 / 27.0 * CA ** 2) * TF * Nf + (44.0 / 9.0 * CF + 158.0 / 27.0 * CA) * TF ** 2 * Nf ** 2
...
def alphas_nloop(mu: float, order: int=0, Nf: int=3) -> float:
    ...
    if order == 2:
        b1 = beta(1, Nf)
        b2 = beta(2, Nf)
        correction = temp + a_s_ref * b1 / b0 * np.log(temp) + a_s_ref ** 2 * (b2 / b0 * (1.0 - 1.0 / temp) + b1 ** 2 / b0 ** 2 * (np.log(temp) / temp + 1.0 / temp - 1.0))
...
def C_ratio(ksi: float, log_scale: float, eps: float=1e-12) -> float:
    ...
    signed_logs = np.sign(ksi) * np.log(np.abs(ksi) + eps) + np.sign(one_minus_ksi) * np.log(np.abs(one_minus_ksi) + eps)
...
def C_hybrid(ksi: float, log_scale: float, y: float, zspz: float, eps: float=1e-12) -> float:
    ...
    delta = 0.5 * (1.0 / (np.abs(one_minus_ksi) + eps) - 2.0 * _sine_integral(one_minus_ksi * wilson_scale) / (np.pi * sign_safe_denominator))
...
def build_matching_matrix(lc_x_ls: np.ndarray, mu: float, quasi_y_ls: np.ndarray | None, eps: float, *, density: DensityFn, color_factor: float=CF, diagonal_extra: Callable[[float], float] | None=None) -> np.ndarray:
...
def _build_pdf_matrix(x_ls: np.ndarray, momentum_gev: float, mu: float, y_ls: np.ndarray | None, eps: float, *, coeff: CoeffFn, plus_coeff: CoeffFn | None=None, color_factor: float=CF, diagonal_extra: Callable[[float], float] | None=None) -> np.ndarray:
    ...
    with _progress_bar(total=2 * nx, desc='matching kernel') as bar:
        ...
        for idx, x_val in enumerate(x_grid):
            ...
            cols_weights = ((pos - 1 if ascending else ny - pos, 1.0 - w_hi), (pos if ascending else ny - 1 - pos, w_hi))
...
def _hybrid_gi_delta(ksi: float, y: float, zspz: float, eps: float, strength: float) -> float:
    ...
    sign_safe_denominator = one_minus_ksi + np.sign(one_minus_ksi) * eps
    wilson_scale = np.abs(y) * zspz
    return strength * (-1.0 / (np.abs(one_minus_ksi) + eps) + 2.0 * _sine_integral(one_minus_ksi * wilson_scale) / (np.pi * sign_safe_denominator))
...
def V_qq_t(x: float, y: float, momentum_gev: float, mu: float, eps: float=1e-12) -> float:
    ...
    return np.abs(x) / (y * (y - x)) * (l_x - 1.0) + np.abs(1.0 - x) / ((1.0 - y) * (x - y)) * (l_xbar - 1.0) + (x + y - 2.0 * x * y) / (np.abs(x - y) * y * (1.0 - y)) * (l_xy - 1.0)

def V_qq_h(x: float, y: float, momentum_gev: float, mu: float, eps: float=1e-12) -> float:
    l_x = _da_log(x, momentum_gev, mu, eps)
    l_xbar = _da_log(1.0 - x, momentum_gev, mu, eps)
    l_xy = _da_log(x - y, momentum_gev, mu, eps)
    brace = np.abs(x) / y * (l_x - 1.0) + np.abs(1.0 - x) / (1.0 - y) * (l_xbar - 1.0) + np.abs(x - y) / (y * (y - 1.0)) * (l_xy - 1.0)
...
def _da_matrix(lc_x_ls: np.ndarray, momentum_gev: float, mu: float, quasi_y_ls: np.ndarray | None, eps: float, *, coefficient: Callable[[float, float, float, float, float], float], wilson_line: Callable[[float, float], float]) -> np.ndarray:
...
def _renormalon_params(nf: int) -> tuple[float, float, float, float]:
    ...
    c2 = (beta1 ** 4 + 4.0 * beta0 ** 3 * beta1 * beta2 - 2.0 * beta0 * beta1 ** 2 * beta2 + beta0 ** 2 * (-2.0 * beta1 ** 3 + beta2 ** 2) - 2.0 * beta0 ** 4 * beta3) / (32.0 * (b - 1.0) * b * beta0 ** 8)
...
def rnasym(n: int, z: float, mu: float, nf: int=3) -> float:
    ...
    return float(_NM_RENORMALON[nf] * abs(z * mu) * (beta0 / (2.0 * np.pi)) ** n * gamma(n + 1.0 + b) / gamma(1.0 + b) * tail)

def dPVasym(z: float, mu: float, nf: int, alphas: float) -> float:
    try:
        import mpmath
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError("The LRR matching kernels need mpmath for the exponential-integral E_nu; install the 'analysis' extra (pip install -e '.[analysis]').") from exc
    beta0, b, c1, c2 = _renormalon_params(nf)
...
def C_z_lrr(ksi: float, y: float, momentum_gev: float, zspz: float, eps_m: float=0.005, eps: float=1e-12) -> float:
    ...
    term2 = emz * pz / (np.pi * denom) * ((M ** 2 - pz ** 2 * om ** 2 + M ** 3 * zs + M * pz ** 2 * om ** 2 * zs) * cos_phi + pz * om * (2.0 * M + M ** 2 * zs + pz ** 2 * om ** 2 * zs) * sin_phi)
...
def _lrr_improve(fixed_order_matrix: np.ndarray, x_ls: np.ndarray, y_ls: np.ndarray, momentum_gev: float, mu: float, zspz: float, eps: float, *, nf: int=3, eps_m: float=0.005, restrict_unit_interval: bool=False) -> np.ndarray:
    ...
    m_cz_exp = m_cz_sum if x_ls.shape == y_ls.shape and np.allclose(x_ls, y_ls, rtol=0.0, atol=1e-12) else _plus_prescription_matrix(y_ls, y_ls, density_cz, eps)
    alpha_s = _alpha_s(float(mu))
...
def _lrr_from_fixed_order(fixed_order_builder: Callable[..., np.ndarray], lc_x_ls: np.ndarray, momentum_gev: float, mu: float, quasi_y_ls: np.ndarray | None, eps: float, zspz: float | None, *, restrict_unit_interval: bool=False) -> np.ndarray:
    if zspz is None:
...
def _p_nlo_full_unpolarized(nu: np.ndarray) -> np.ndarray:
    ...
    value = 4.0 * ca * cf * (a * (67.0 / 18.0 - z2 + 11.0 / 6.0 * ln + ln ** 2 / 2.0) + b * (z2 + 2.0 * (ln * ln1p + _dilog(-v)) - ln ** 2 / 2.0) + 14.0 / 3.0 * (1.0 - v)) - 4.0 * cf * nf * (a * (5.0 / 9.0 + 1.0 / 3.0 * ln) + 2.0 / 3.0 * (1.0 - v)) + 4.0 * cf ** 2 * (2.0 * a * (-ln * ln1m - _dilog(v) - 0.75 * ln + _dilog(v)) - 2.0 * b * (z2 + 2.0 * (ln * ln1p + _dilog(-v)) - ln ** 2 / 2.0) - (1.0 - v) * (1.0 - 1.5 * ln) - ln - (1.0 + v) * ln ** 2 / 2.0)
    return np.where(inside, value, 0.0)
...
def _c_parity_term(nu: np.ndarray) -> np.ndarray:
    ...
    value = 16.0 * cf * (cf - ca / 2.0) * (b * (z2 + 2.0 * (ln * ln1p + _dilog(-v)) - ln ** 2 / 2.0) - 2.0 * (1.0 - v) - (1.0 + v) * ln)
    return np.where(inside, value, 0.0)
...
def _p_nlo_transversity(nu: np.ndarray) -> np.ndarray:
    ...
    cf, ca, nf = (_SPLIT_CF, _SPLIT_CA, _SPLIT_NF)
    q = 4.0 * v / (1.0 - v)
    value = ca * cf * (-2.0 * (1.0 - v) + q * (ln ** 2 + 11.0 / 3.0 * ln + 67.0 / 9.0 - np.pi ** 2 / 3.0)) - cf * (nf / 2.0) * (4.0 / 3.0 * q * (ln + 5.0 / 3.0)) + cf ** 2 * (4.0 * (1.0 - v) - q * (3.0 * ln + 4.0 * ln * ln1m)) + 4.0 * (cf ** 2 - ca * cf / 2.0) * (-(1.0 - v) + 4.0 * v / (1.0 + v) * (ln ** 2 / 2.0 - np.pi ** 2 / 6.0 - 2.0 * _dilog(-v) - 2.0 * ln * ln1p))
    return np.where(inside, value, 0.0)
...
def _dglap_evolution_matrices(x_grid: np.ndarray, p_nlo: Callable[[np.ndarray], np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    x = np.asarray(x_grid, dtype=float)
...
def _evolution_operator(mu_initial: float, mu_final: float, evo_lo: np.ndarray, evo_nlo: np.ndarray, steps: int) -> np.ndarray:
    from scipy.linalg import expm
...
def _rgr_from_fixed_order(fixed_order_builder: Callable[..., np.ndarray], p_nlo: Callable[[np.ndarray], np.ndarray], lc_x_ls: np.ndarray, momentum_gev: float, mu: float, quasi_y_ls: np.ndarray | None, eps: float, zspz: float | None, *, needs_zspz: bool, takes_zspz: bool, needs_zpsi: bool, kappa: float=1.0, mu_min: float=0.6, steps: int=20) -> np.ndarray:
