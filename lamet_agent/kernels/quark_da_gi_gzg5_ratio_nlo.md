...
- **Delta term**: There is no explicit $\delta(x-y)$ term in the paper's Eq. (4.15); the code's `build_matching_matrix_column_plus` includes the LO delta via interpolation, and the plus-prescription handles the singularity. No discrepancy.
